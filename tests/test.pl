...
sub execute_command {
	...
	TODO: {
		...
		open(RDFH, "$command |") or die "Failed to execute command " .
						"$cmdcounter: '$command'";
...
foreach (@TESTDATA) {
	...
	if (s/^default\s+//) {
		...
		is($in, $output{'rec-out'}, "rec-out-auto: $record");
	}
	else {
